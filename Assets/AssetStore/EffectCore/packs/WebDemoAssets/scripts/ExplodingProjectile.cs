...
public class ExplodingProjectile : MonoBehaviour
{
    ...
    void Start()
    {
        attackPoint = GameObject.Find("Player").GetComponent<PlayerController>().playerStatus.damage;
        ...
        thisCollider.size = new Vector3(0.3f,0.21f,0.33f);
    ...
    void CheckCollision(Vector3 prevPos)
    {
        ...
        if (Physics.Raycast(ray, out hit, dist,this.LayerMask))
        {
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.gameObject.tag != "AI")
        {
            ...
            if(collision.collider.gameObject.transform.parent != null ){
                if (collision.collider.gameObject.transform.parent.tag == "Enemy")
                {
                    collision.collider.gameObject.transform.parent.GetComponent<Enemy>().TakeDamage(attackPoint);
                }
            }
